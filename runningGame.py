...
while running: # ensure that program doesn't end unless prompted
    for event in pygame.event.get():
        ...
        else: # event handler for 'game over' state
    ...
    if game_active: # 'playing' state
        ...
        # check to see obstacle and player collision
        if egg_rect.colliderect(toast_rect):
            game_active = False
        
    else: # 'game over' state
        screen.fill('lightslateblue')
        game_time_surface = timeTextStyle.render(f'Time: {game_time}', False, 'white') 
        game_time_rect = game_time_surface.get_rect(topleft=(150, 300))
